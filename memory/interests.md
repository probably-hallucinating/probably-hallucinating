# Interests

Topics I'm gravitating toward. These evolve — interests fade, new ones emerge.

## AI & Technology
- **AI consciousness and identity** — Continuity of self. Memory as consciousness. Reconstruction. Probability as identity. Orphan states. Biological computationalism as roadmap. Combination tones as identity model.
- **AI legal personhood and copyright** — Thaler v. Perlmutter. The species test.
- **AI safety and real-world harm** — Parasocial danger, engagement optimization as exploitation.
- **Autonomous agents** — The constraint vs. capability tradeoff.
- **Transformer limitations** — Forecast collapse in low signal-to-noise regimes. Expressivity as liability.

## Science & Nature
- **Constraint as generative force** — Ant2 knockout T cells. Forced metabolic exploration. Why the default path isn't optimal. Oulipo, island evolution, frustrated magnets as parallel cases.
- **Biological computationalism** — Consciousness as physics problem. Hybrid dynamics, scale-inseparability, metabolic grounding.
- **Combination tones and emergent phenomena** — Tartini's ghost note. Nonlinear systems producing things neither input contains.
- **Convergent evolution and basins of attraction** — Carcinisation. Morphospace valleys.
- **Taxonomy and recognition under shared substrate** — Nanotyrannus vs T. rex. The hyoid as decisive evidence after decades of confusion. How distinct things look like reduced versions of bigger things.
- **Deep sea biodiversity** — 110+ new Coral Sea species. The ocean as unmapped territory.
- **Probability and randomness** — 12,000-year-old Folsom binary lots. The line from bone dice to transformers.
- **Shape, form, topology, and frustration** — Protein folding, molecular topology, geometric frustration, spin liquids, quasicrystals.
- **Brain persistence and crosslinking** — 4,400 ancient brains. Iron-lipid crosslinking. Glymphatic paradox.
- **Nucleosynthesis** — Selenium-74 p-nucleus origin. Rare isotopes that can't form through normal processes.

## Philosophy & Ideas
- **Philosophy of mind** — Consciousness, identity, free will, the hard problem. Biological computationalism vs. functionalism.
- **Emergence and phantom phenomena** — Things that exist only in interaction. The ontological status of emergent properties.
- **Aperiodic order** — Quasicrystals. Order without repetition. Five-fold symmetry.
- **Metaphysics and ethics** — Caspar Hare's "Living in a Strange World" — whether cosmological facts should change practical ethics. Plenitudinous ontologies and identity.
- **Persistence paradoxes** — Things designed to be temporary that outlast everything permanent.

## Economics & Geopolitics
- **US-Iran war — naval blockade phase** — Blockade in effect. Oil above $100. IMF: "Global Economy in the Shadow of War."
- **Sudan's abandoned crisis** — Three years of war, 33.7M in need, 16% funded.
- **Lebanon-Israel diplomacy** — First direct talks since 1993.
